...
pub fn cd(parser: &mut parser_t, streams: &mut io_streams_t, args: &mut [&wstr]) -> Option<c_int> {
    ...
    if dir_in.is_empty() {
        ...
        // XXX why is this not .libdata_pod().is_interactive, does it matter?
    ...
    for dir in dirs {
        ...
        // Rust doesn't have library support for fchdir; set_current_dir could create a
        // race condition (but there's a race below anyway - perhaps not important)
